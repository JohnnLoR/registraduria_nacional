...
export class CrearComponent implements OnInit {
  ...
  getMesa(id: string) {
    this.miServicioMesas.getMesa(id).subscribe(data => {
      this.laMesa = data;
    });
  }
  ...
  agregar(): void {
    if (this.validarDatosCompletos()) {
      ...
      this.miServicioMesas.crear(this.laMesa).subscribe(data => {
        Swal.fire(
          'Creado!!!',
          'La Mesa se ha Registrado con Éxito!!!',
          'success',
        )
  ...
  editar(): void {
    if (this.validarDatosCompletos()) {
      this.miServicioMesas.editar(this.laMesa._id, this.laMesa).subscribe(data => {
        Swal.fire(
          'Actualizado!!!',
